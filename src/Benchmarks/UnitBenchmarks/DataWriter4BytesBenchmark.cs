...
public class DataWriter4BytesBenchmark
{
    private byte[] _array = null!;
    private RecyclableMemoryStream _stream = null!;
    private List<Memory<byte>> _list = null!;
    ...
    public void Setup()
    {
        ...
        _array = new byte[512];
        _list = new List<Memory<byte>>();
        new Random(1996).NextBytes(_array);

        for (var i = 0; i < _array.Length - 3; i += 4)
        {
            _list.Add(new Memory<byte>(_array, i, 4));
        }
    }
